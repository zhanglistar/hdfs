...
%.pb.go: $(HADOOP_HDFS_PROTOS) $(HADOOP_COMMON_PROTOS)
	@for i in `ls internal/protocol/hadoop_hdfs/*.proto`; do protoc --go_out=paths=source_relative:./internal/protocol/hadoop_hdfs -Iinternal/protocol/hadoop_common -Iinternal/protocol/hadoop_hdfs  $$i ;done
	@for i in `ls internal/protocol/hadoop_common/*.proto`; do protoc --go_out=paths=source_relative:./internal/protocol/hadoop_common -Iinternal/protocol/hadoop_common -Iinternal/protocol/hadoop_hdfs  $$i ;done
